...
*[This response was generated by Twilio Studio using keyword matching - no AI/LLM involved]*`
    ...
    return `🤖 **Simple Chatbot Response**
...
*[This is a Twilio Studio bot using keyword matching - not an AI/LLM]*`
  ...
  const toggleAIMode = () => {
    ...
    if (newMode === 'twilio') {
      setMessages([{
        id: Date.now().toString(),
        content: "🔄 **Switched to Advanced Chatbot** Hi! I'm your Soles4Souls AI assistant powered by Twilio AI Assistants. I can help you with donations, volunteer opportunities, impact tracking, drop-off locations, and more. How can I assist you today?",
        sender: 'assistant',
        timestamp: new Date().toISOString(),
      }])
    } else {
      setMessages([{
        id: Date.now().toString(),
        content: "🔄 **Switched to Simple Chatbot** I'm a Twilio Studio-powered chatbot that only knows about Nashville donation drop-offs. I use keyword matching to provide responses - no AI/LLM involved. Try asking about 'donation in Nashville' or 'Nashville drop-off hours'!",
        sender: 'assistant',
        timestamp: new Date().toISOString(),
      }])
    }
